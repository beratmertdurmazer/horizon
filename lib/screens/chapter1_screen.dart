...
class _Chapter1ScreenState extends State<Chapter1Screen> with TickerProviderStateMixin {
  // Sabitleşen Seri: Y5 -> B8 -> G14 -> P26 -> L50 -> Cevap: D98
  final List<String> _sequence = ["Y5", "B8", "G14", "P26", "L50"];
  final String _correctAnswer = "D98";
  ...
  void _generateBalancedOptions() {
    // Kullanıcının istediği: 12 seçenek, her harf ve rakamdan 2şer adet (dengeli çeldiriciler)
    // D98, D100, F100, F86, B86, B44, E44, E110, G110, G50, H50, H98
    _options = [
      "D98", // Doğru
      "D100", "F100", "F86", "B86", "B44", "E44", "E110", "G110", "G50", "H50", "H98"
    ];
    _options.shuffle();
  }
  ...
  void _startNarrativeTyping() {
    ...
    _typewriterTimer = Timer.periodic(const Duration(milliseconds: 40), (timer) {
      if (charIndex < _fullNarrative.length) {
        setState(() {
          _displayedText += _fullNarrative[charIndex];
          charIndex++;
        });
        ...
        timer.cancel();
        setState(() {
          _isTypingNarrative = false;
          _statusText = "SİSTEM HAZIR. DEŞİFRE İÇİN BEKLENİYOR.";
        });
        _stopwatch.start();
        _startHeartbeat();
  ...
  void _startHeartbeat() {
    ...
    _heartbeatTimer = Timer(Duration(milliseconds: ms), () {
      if (!_isCompleted && !_isTypingNarrative) {
  ...
  void _onOptionTap(String value) {
    ...
    if (value == _correctAnswer) {
      ...
      setState(() {
        _isCompleted = true;
        _statusText = "SİNAPTİK BAĞLANTI KURULDU.";
        _stopwatch.stop();
        _completeChapter();
      });
    } else {
      _triggerGlitch();
      setState(() {
        _statusText = "HATA: HATALI BİLEŞEN ALGILANDI!";
      });
    }
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Positioned.fill(
            child: Image.asset(
              "images/concept_cyber_noir_cold_awakening_128391231231_1774315250438.png",
              fit: BoxFit.cover,
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!_isTypingNarrative) ...[
                        ...
                        Expanded(child: _buildOptionsGrid(dynamicColor)),
                      ] else 
              ...
              child: Container(
                decoration: BoxDecoration(
                  gradient: RadialGradient(
                    colors: [
                      Colors.transparent,
                      Colors.red.withOpacity(0.25 + (stressPulse * 0.35)),
                    ],
                    stops: const [0.4, 1.0],
                  ),
                ),
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              ...
              children: [
                ..._sequence.map((s) => _buildDataPacket(s, color)).toList(),
                _buildDataPacket("??", color, isTarget: true),
              ],
            ),
          ),
        ],
  ...
  Widget _buildDataPacket(String label, Color color, {bool isTarget = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: isTarget ? color.withOpacity(0.15) : Colors.black.withOpacity(0.6),
        border: Border.all(color: isTarget ? color : color.withOpacity(0.3), width: isTarget ? 2 : 1),
        borderRadius: BorderRadius.circular(4),
      ),
      child: Text(
        label,
  ...
  Widget _buildOptionsGrid(Color color) {
    return GridView.builder(
      itemCount: 12, // 12 SEÇENEK
      physics: const NeverScrollableScrollPhysics(),
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 3, // 3 SÜTUN (3x4)
        crossAxisSpacing: 8,
        mainAxisSpacing: 8,
  ...
  Widget _buildFooterStatus(Color color, double pulse) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            _isTypingNarrative ? Icons.keyboard : Icons.biotech, 
            size: 18, 
